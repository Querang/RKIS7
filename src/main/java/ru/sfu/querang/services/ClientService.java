...
public class ClientService {
    ...
    public Mono<List<Car>> getFilteredByPrice(Float price) {
        return webClient.get().uri(uriBuilder -> uriBuilder.path("/cars/api").queryParam("price", price).build())
                .retrieve()
    ...
    public void create(Car car) {
        webClient.post().uri("/cars/api").contentType(MediaType.APPLICATION_JSON).body(Mono.just(car), Car.class)
                .retrieve()
    ...
    public void update(int id, Car updatedCar) {
        webClient.put().uri("/cars/api/{id}", id).contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromValue(updatedCar))
                .retrieve()
                .bodyToMono(Void.class)
                .block();
    }
